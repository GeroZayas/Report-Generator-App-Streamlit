...
def main():
    ...
    st.markdown("### Select Directory to Save Report")
    directory_path = st.file_uploader("Choose a file in the desired directory")
    ...
    if st.button("Generate Report"):
        ...
        if general_marks_valid and exam_marks_valid:
            if directory_path is not None:
                save_dir = os.path.dirname(directory_path.name)
                ...
                st.error("Please select a directory to save the report.")
...
def generate_report(
    ...
    doc.render(values)
    output_path = os.path.join(save_dir, f"{values['STUDENT']}-{values['LEVEL']}.docx")
